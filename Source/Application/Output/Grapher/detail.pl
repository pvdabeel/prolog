...
emit_legend :-
    ...
    write('  <span class="legend-item"><span class="legend-swatch" style="background:var(--surface2);border:1px solid var(--border2)"></span> candidate</span>'), nl,
    write('  <span class="legend-item"><span class="legend-swatch" style="background:var(--green-bg);border:1px solid var(--green-border)"></span> installed</span>'), nl,
    write('</div>'), nl.
...
emit_js_graph_renderer :-
    ...
    write('  candidate:      {fill:"var(--surface2)",  stroke:"var(--border2)", text:"var(--text2)"},'), nl,
    write('  candidate_inst: {fill:"var(--green-bg)",  stroke:"var(--green-border)", text:"var(--green)"}'), nl,
    write('};'), nl,
    ...
    write('    if (node.candidates && node.candidates.length) {'), nl,
    write('      node.candidates.forEach((c, ci) => {'), nl,
    write('        const cid = id + "_c" + ci;'), nl,
    write('        const cType = c.inst ? "candidate_inst" : "candidate";'), nl,
    write('        const parts = c.id.split("/");'), nl,
    write('        const cLabel = parts.length > 1 ? parts[1] : c.id;'), nl,
    write('        flat.push({id: cid, nodeType: cType, label1: cLabel, label2: c.inst ? "installed" : "", depth: depth + 1});'), nl,
    write('        edges.push({from: id, to: cid, style: "dotted"});'), nl,
    write('      });'), nl,
    write('    }'), nl,
...
emit_js_graph_pan_zoom :-
    ...
    write('  const svg = document.getElementById("graph-svg");'), nl,
    write('  let vbX=0,vbY=0,vbW=0,vbH=0,lastDx=0,lastDy=0,ctmA=1,ctmD=1;'), nl,
    write('  function syncVB(){const v=svg.getAttribute("viewBox");if(!v)return false;const p=v.split(" ");vbX=+p[0];vbY=+p[1];vbW=+p[2];vbH=+p[3];return true;}'), nl,
    write('  function applyVB(){svg.setAttribute("viewBox",vbX+" "+vbY+" "+vbW+" "+vbH);}'), nl,
    write('  function endPan(){'), nl,
    write('    if(!isPanning)return;isPanning=false;container.style.cursor="";'), nl,
    write('    vbX=vbStart.x-lastDx/ctmA;vbY=vbStart.y-lastDy/ctmD;applyVB();'), nl,
    write('    svg.style.transform="";svg.style.willChange="";'), nl,
    write('    lastDx=0;lastDy=0;'), nl,
    write('  }'), nl,
    write('  container.addEventListener("mousedown", e => {'), nl,
    write('    if (e.target.closest("g")) return;'), nl,
    write('    if (!syncVB()) return;'), nl,
    write('    isPanning = true; lastDx = 0; lastDy = 0;'), nl,
    write('    panStart = {x: e.clientX, y: e.clientY};'), nl,
    write('    vbStart = {x: vbX, y: vbY};'), nl,
    write('    const ctm = svg.getScreenCTM();'), nl,
    write('    if (ctm) { ctmA = ctm.a; ctmD = ctm.d; }'), nl,
    write('    svg.style.willChange = "transform";'), nl,
    write('    container.style.cursor = "grabbing";'), nl,
    write('  });'), nl,
    write('  container.addEventListener("mousemove", e => {'), nl,
    write('    if (!isPanning) return;'), nl,
    write('    lastDx = e.clientX - panStart.x;'), nl,
    write('    lastDy = e.clientY - panStart.y;'), nl,
    write('    svg.style.transform = "translate(" + lastDx + "px," + lastDy + "px)";'), nl,
    write('  });'), nl,
    write('  container.addEventListener("mouseup", endPan);'), nl,
    write('  container.addEventListener("mouseleave", endPan);'), nl,
    ...
    write('    const nw = vbW / factor, nh = vbH / factor;'), nl,
    write('    vbX += (vbW - nw) * mx;'), nl,
    write('    vbY += (vbH - nh) * my;'), nl,
    write('    vbW = nw; vbH = nh;'), nl,
    write('    applyVB();'), nl,
