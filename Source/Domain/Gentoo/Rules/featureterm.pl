...
%! featureterm:ctx_take_after(+Context0, -After, -Context)
%
...
%! featureterm:ctx_take_after_with_mode(+Context0, -After, -AfterForDeps, -Context)
%
...
%! featureterm:ctx_add_after_condition(+After, +AfterForDeps, +Conds0, -Conds)
%
...
featureterm:ctx_add_after_condition(none, _AfterForDeps, Conditions, Conditions) :- !.
featureterm:ctx_add_after_condition(After, none, Conditions0, [constraint(order_after(After):{[]} )|Conditions0]) :-
  After \== none,
  !.
featureterm:ctx_add_after_condition(After, _AfterForDeps, Conditions0, [After|Conditions0]) :-
  !.


%! featureterm:ctx_strip_planning(+Context0, -Context)
%
...
featureterm:add_after_to_dep_contexts(After, Deps0, Deps) :-
  ...
  findall(D,
          ( member(D0, Deps0),
            ( D0 = Term:Action?{Ctx0} ->
                featureterm:ctx_add_after(Ctx0, After, Ctx),
                D = Term:Action?{Ctx}
            ; D = D0
            )
          ),
...
featureterm:add_after_only_to_dep_contexts(After, Deps0, Deps) :-
  ...
  findall(D,
          ( member(D0, Deps0),
            ( D0 = Term:Action?{Ctx0} ->
                featureterm:ctx_add_after_only(Ctx0, After, Ctx),
                D = Term:Action?{Ctx}
            ; D = D0
            )
          ),
...
featureterm:drop_build_with_use_from_dep_context(Dep0:Act?{Ctx0}, Dep:Act?{Ctx}) :-
  !,
  Dep = Dep0,
  featureterm:ctx_drop_build_with_use(Ctx0, Ctx).
...
featureterm:ctx_drop_build_with_use(Ctx0, Ctx) :-
  ( is_list(Ctx0) ->
      exclude(featureterm:ctx_is_build_with_use_term, Ctx0, Ctx)
  ; Ctx = Ctx0
  ),
  !.

featureterm:ctx_is_build_with_use_term(build_with_use:_) :- !.


%! featureterm:ctx_drop_assumption_reason(+Ctx0, -Ctx)
%
% Removes all `assumption_reason(_)` terms from a context.

featureterm:ctx_drop_assumption_reason(Ctx0, Ctx) :-
  ( is_list(Ctx0) ->
      exclude(featureterm:ctx_is_assumption_reason_term, Ctx0, Ctx)
  ; Ctx = Ctx0
  ),
  !.

featureterm:ctx_is_assumption_reason_term(assumption_reason(_)) :- !.


%! featureterm:ctx_drop_build_with_use_and_assumption_reason(+Ctx0, -Ctx)
%
...
featureterm:ctx_drop_build_with_use_and_assumption_reason(Ctx0, Ctx) :-
  ( is_list(Ctx0) ->
      exclude(featureterm:ctx_is_bwu_or_assumption_reason, Ctx0, Ctx)
  ; Ctx = Ctx0
  ),
  !.

featureterm:ctx_is_bwu_or_assumption_reason(build_with_use:_) :- !.
featureterm:ctx_is_bwu_or_assumption_reason(assumption_reason(_)) :- !.