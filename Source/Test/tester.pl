...
tester:test(single_verbose,Name,Repository://Item,Generator,Test,Report,Scroll) :-
  ...
  forall(Generator,
         ( catch(
             ...
             ( message:scroll_failure([Item,' (time limit exceeded)']),
              ...
              ( current_predicate(sampler:record/1) ->
                  sampler:record(failed(timeout))
              ; true
              ),
              ( current_predicate(sampler:record/1) ->
                  sampler:record(failed_entry(Repository://Item, timeout))
              ; true
              )
            )
          )
        ; message:clean,
          ...
          ( current_predicate(sampler:record/1) ->
              sampler:record(failed_entry(Repository://Item, other))
          ; true
          )
         )),
  !,
...
tester:test(parallel_verbose,Name,Repository://Item,Generator,Test,Report,Scroll) :-
  ...
  concurrent_forall(Generator,
                      ...
                      ( % Never let an exception from a worker thread abort the whole test run.
                        ...
                        ( current_predicate(sampler:record/1) ->
                            sampler:record(failed(exception))
                        ; true
                        ),
                        ( current_predicate(sampler:record/1) ->
                            sampler:record(failed_entry(Repository://Item, exception))
                        ; true
                        )
                      )
                    ),
