...
run_variants(VariantsOpt, Proposal, BaseProof, BasePlan, _BaseTriggers) :-
...
print_variant_results([], _, _).

print_variant_results([variant_result(Spec, failed)|Rest], BaseEntries, N) :-
  !,
...
print_variant_results([variant_result(Spec, _Proof, _Model, Plan, _Triggers)|Rest], BaseEntries, N) :-
  Spec = variant(_, _, _, _, Label),
  nl,
