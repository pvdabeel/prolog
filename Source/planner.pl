...
Wave planner (Kahn's algorithm): all non-constraint deps are hard ordering
constraints.  The planner schedules the acyclic portion of the dependency
graph.  Any rules left unscheduled (typically due to cycles) are returned as
the remainder for the scheduler, which performs SCC-aware progressive
relaxation (relax :run edges within cycles, matching PMS/Portage semantics).
*/
...
planner:plan(ProofAVL, TriggersAVL, PlannedHeadsIn, EnrichedPlan, RemainderRules) :-
    ...
    (   ReadyQueue0 \= [] ->
        ...
        plan_loop([ReadyQueue0], DepCounts0, TriggersAVL, ProofAVL, PlannedHeadsInit,
                  [], RevPlan, DepCounts, PlannedHeads),
        reverse(RevPlan, EnrichedPlan)
    ;   EnrichedPlan = [],
        DepCounts = DepCounts0,
        PlannedHeads = PlannedHeadsIn
    ),
