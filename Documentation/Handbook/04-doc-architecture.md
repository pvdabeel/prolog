...
loads a mode's groups from `loader:mode/2`, then the optional groups
from `loader:optional/3` whose condition holds (the LLM groups, gated
by `config:load_llm_modules/1`):
